...
reg add HKLM\System\Setup /v SetupType /t REG_DWORD /d 3 /f > nul
reg add HKLM\System\Setup /v SetupSupported /t REG_DWORD /d 1 /f > nul
reg add HKLM\System\Setup /v SetupPhase /t REG_DWORD /d 2 /f > nul
reg add HKLM\System\Setup /v RestartSetup /t REG_DWORD /d 1 /f > nul
...
if exist %systemdrive%\dosexec.bat (
	echo.
	echo Done, rebooting.
...
if not exist %systemdrive%\dosexec.bat (
	...
	@echo reg add HKLM\System\Setup /v SystemSetupInProgress /t REG_DWORD /d 1 /f >> %systemdrive%\dosexec.bat
	@echo net start >> %systemdrive%\dosexec.bat 
	@echo ping localhost -n 3 >> %systemdrive%\dosexec.bat 
	@echo cls >> %systemdrive%\dosexec.bat
	@echo echo The system runs in MS-DOS Mode, type "win" to get back to Windows. >> %systemdrive%\dosexec.bat
	echo.
	echo =-=-=-=-=-= DOSEXEC.BAT =-=-=-=-=-=
